...
<nzb xmlns="http://www.newzbin.com/DTD/2003/nzb">
 <head>
   ...
   <meta type="password">MySecretPass</meta>
 </head>
...
func TestNzbParse(t *testing.T) {
    nzb, err := FromString(testNzbXml)
    if err != nil {
        t.Fatalf("expected to parse nzb data: %s", err)
    }
    ...
    if f.Poster != "Joe Bloggs <bloggs@nowhere.example>" {
        t.Errorf("expected poster to be Joe Bloggs <bloggs@nowhere.example> but got %v", f.Poster)
    }
    if len(f.Groups) != 2 {
